...
As manmade terrain class is near the lidar in every scene, it is common with outdoor lidar datasets.
...
The scans are generated using a Matterport 3D scanner as a triangular mesh and can be seen in Figure~\ref{fig:how_matterport_works}.
...
Along with ShapeNet dataset \cite{chang2015shapenet}, S3DIS is also one of the most evaluated datasets in indoor settings for semantic segmentation using in \cite{Armeni_2016_CVPR_S3DIS} whereas ShapeNet is used for part segmentation.
\begin{figure}[!ht]
    \centering
    \begin{tabular}{cc}
...
So we removed the colours of the Semantic3D dataset, and since the model requires RGB values for prediction, we changed the colour of points to black.
\end{document}