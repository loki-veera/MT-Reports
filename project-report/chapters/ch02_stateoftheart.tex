...
\begin{document}
    ...
    This kind of dataset captures the static information of the real worldIn contrast, the sequential datasets capture the dynamic movements of the surrounding objects.
...
The first of this kind is SqueezeSeg, proposed in \cite{Sequeseseg_2018}. It employs SqueezeNet for 2D segmentation and then reprojected to 3D using Recurrent Conditional Random Fields.
...
Finally, graph neural networks \cite{dyn_graph_cnn} can also produce better segmentation results. However, this study is limited to standard deep learning models, so this study does not include graph neural networks.
    ...
    \cite{matias_uncertainty}  argues that epistemic uncertainty is most helpful in OOD detection.
    ...
    Another advantage of using deep ensembled is the lowest correlation between the model instances as the training of the instances is done differently.
    ...
    All other flavours of deep ensembles are proposed to reduce the effect on time, but the memory requirements remain mostly the same except for the subensembles.
    ...
    As in \cite{bhandary2020evaluating} and \cite{gawlikowski2021survey} training with dropout layers can be formulated as VI, and predictive uncertainty can be calculated by applying MC-Dropout during inference..
    ...
    The most widely used benchmark datasets used for 2D classification are CIFAR-10 vs SVHN \cite{liang2017enhancing_ODIN}, CIFAR-10 vs LSUN \cite{hendrycks2016baseline_MSP}, and MNIST vs not-MNIST \cite{hendrycks2016baseline_MSP}. 
    ...
    TThe OOD data samples have higher uncertainty than ID samples, allowing us to use the uncertainty estimation methods for OOD detection.
